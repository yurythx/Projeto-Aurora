...
|---|---|
| e-MAG 2.0 / WCAG 2.1 AA (Lei 13.146/2015) | `/acessibilidade` (com Declaração de Acessibilidade), `components/layout/EMagAccessibilityBar.tsx`, `eslint-plugin-jsx-a11y` no CI |
...
- [ ] Laudo de **avaliação e-MAG por avaliador humano** com tecnologia assistiva (F5.2) + **VPAT**.
- [ ] **RIPD oficial** assinado pelo DPO (a partir de `docs/RIPD_MODELO.md`).
- [ ] Configuração de **object-lock/retenção** no bucket `audit-worm/` (infra).
- [ ] Definição da **política de retenção** por categoria de dado.
